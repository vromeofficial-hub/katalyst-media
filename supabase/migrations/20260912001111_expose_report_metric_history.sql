-- Keep the token-gated report payload minimal while exposing the aggregate
-- history already stored for each client-visible result metric.
...
comment on function private.fetch_shared_report_impl(text) is
  'Unexposed privileged reader for token-gated active campaign reports; returns only client-visible fields, aggregate metric history, and public post keys.';
